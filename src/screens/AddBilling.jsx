...
import { AddBillingForm } from '../containers/AddBillingForm';
...
export class AddBilling extends React.Component {
	...
	handleFormSubmit(e) {
		...
		Appointment.addBilling(dataJson).then(response => {
			if (response.status === 200) {
				...
				alert(`Sukses tambahkan billing ${this.state.pasien.nama}`)
			} else {
				this.setState({
					loading: false
				})
				alert(`Gagal tambahkan billing ${this.state.pasien.nama}`)
			}
		})
	}
	...
	render() {
		if (this.state.loading) {
			...
			return (
				<AddBillingForm pasien={this.state.pasien} onSubmit={this.handleFormSubmit} />
			)
		}
	}
}